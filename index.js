...
async function startHisoka() {
    const hisoka = makeWASocket({
        ...
        browser: ['walker md','Safari','1.0.0'],
        auth: state
    })
    ...
    hisoka.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            lastDisconnect.error?.output?.statusCode !== DisconnectReason.loggedOut ? startHisoka() : console.log('Disconnected Connection...')
        }
        console.log('Connected Connection...', update)
    })
